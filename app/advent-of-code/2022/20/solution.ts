...
export const performEncryptionMix = (orderedList: OrderedListItem[], actions: ActionListItem[]) => {
...
export const solutionOne = () => {
  let { actions, orderedList } = constructLists(data.map(s => parseInt(s)))
...
export const solutionTwo = () => {
  ...
  let { actions, orderedList } = constructLists(data.map(d => parseInt(d) * encryptionKey))
  for (let x = 0; x < 10; x++) {
    orderedList = performEncryptionMix(orderedList, actions)
  }
